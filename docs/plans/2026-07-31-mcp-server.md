...
**Name resolved (2026-08-01, release prep):** open question 1 and assumption 7 are closed — the published name is **`@frontsail-ai/qr-mcp`** (hyphen; the owned npm org is `frontsail-ai`, and the name is verified free). References above to `@frontsail/qr-mcp` are the plan as written and are left as history. The private `@frontsail/qr-core` keeps its name; it is bundled and never published, so it is not affected. Assumption 7's blocker has since cleared on its own: `npm whoami` authenticates and `npm org ls frontsail-ai` reports `korya - owner`, so the scope is confirmed usable. Publishing additionally requires an OTP — the account is on `auth-and-writes` 2FA, so `npm publish` cannot run unattended.

**Dogfooding found a rendering bug outside this package (2026-08-01).** Pointing a real agent at the built server — as opposed to the SDK-client integration tests — surfaced that the two linear gradient options were named for directions they did not render: `linear-bl-tr` produced top-left→bottom-right and `linear-tl-br` produced top-right→bottom-left. The cause was in `packages/core`, predating the MCP server: the rotation constants were derived with y pointing up, while qr-code-styling reads them in SVG coordinates with y pointing down. The web app's own preview chip uses CSS `linear-gradient(45deg/135deg)`, which honours the labels, so the swatch and the generated QR had contradicted each other in production since the feature shipped. Fixed separately by correcting the constants to `-π/4` / `+π/4`, which makes the QR match the label, the icon and the chip; the `gradient-linear` parity golden was recaptured from the corrected web app. Lesson worth keeping: the SDK-client tests could not have caught this, because every one of them asserted against the same wrong source of truth.
